...
/*
Вопросы:
1) При вызове Popover-а на событие и закрытии его появляется popover на добавление события не этот же день.
    Я использовал везде stopPropagation, но это не предотвращает распространение события дальше.
2) При клике на любое поле изменения события, его поповер закрывается, и вылезает поповер на добавление события.
3) В консоле логгирование отображает, что функция, вычисляющая popover запускается 2 раза. Тоже не ясно, почему.
*/
